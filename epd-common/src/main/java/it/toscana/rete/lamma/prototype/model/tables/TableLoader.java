...
public class TableLoader {
	...
	public static WaveresGenericTable laodWaveGenericTable(Path path) throws RouteLoadException {
		...
		try {
            ...
            pdsComp = parseDimension(line);


            int pdsMin, pdsMax, pdsDelta;
            pdsMin = (int) pdsComp.getMin();
            pdsMax = (int) pdsComp.getMax();
            pdsDelta = (int) pdsComp.getDelta();
            int tMSize = tM.getSize();
            
            for(int i = 0; i <spd.getSize(); i++) {
            	waveGeneric = waveresg.addWavePeriods(new WavePeriods(tM,pdsComp), i);
           
            	for(int n =0; n < tM.getSize(); n++) {
            		line = reader.readLine().trim(); 
            		waveGeneric.addCawValue(new OneDimDouble(pdsMin, pdsMax, pdsDelta, parseValues(line)), n);
	...
	public static WaveresCompTable laodWaveCompTable(Path path) throws RouteLoadException {
		...
		try {
            ...
            int cawMin, cawMax, cawDelta;
            cawMin = (int) caw.getMin();
            cawMax = (int) caw.getMax();
            cawDelta = (int) caw.getDelta();
            int tPSize = tP.getSize();
            ...
            for(int i = 0; i <spd.getSize(); i++) {
            	...
            	for(int n =0; n < tP.getSize(); n++) {
            		line = reader.readLine().trim(); 
            		waveGeneric.addCawValue(new OneDimDouble(cawMin, cawMax, cawDelta, parseValues(line)), n);
	...
	public static FuelRateTable laodFuelRateTable(Path path) throws RouteLoadException {
		...
		try {
            ...
            fuelRateTable = new FuelRateTable(spd, fuelRate);
